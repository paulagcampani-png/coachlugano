...
const CoachFraAnalytics = (() => {
  ...
  function trackFormSubmit(formData) {
    const goalsList = Array.isArray(formData.goals) ? formData.goals : [];
    pushEvent("generate_lead", {
      form_id: "contact_form",
      lead_goals: goalsList.length ? goalsList.join(",") : "not_specified",
      lead_goal_count: goalsList.length,
      // NOTE: do not push personally identifiable info (name, email, phone)
      // into the dataLayer/GA4. Keep PII inside Formspree only.
    });
  }

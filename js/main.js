...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (form) {
    ...
    form.addEventListener("submit", async (e) => {
      ...
      const goals = formData.getAll("goals"); // array of checked goal values
      const firstName = formData.get("firstName");
      const email = formData.get("email");

      if (!firstName || !email || goals.length === 0) {
        formStatus.textContent = "Please fill in the required fields (first name, email, and at least one goal).";
      ...
      try {
        ...
        CoachFraAnalytics.trackFormSubmit({ goals });
        form.reset();
        startTracked = false;
      } catch (err) {

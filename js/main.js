...
document.addEventListener("DOMContentLoaded", () => {
  ...
  /* ---------- Mobile nav ----------
     The mobile panel ships with inline style="display:none" in the HTML
     itself (not just a CSS class), so JS must explicitly set display:flex
     to show it and display:none to hide it — toggling only a CSS class is
     not enough here, since the inline style would otherwise always win. */
  ...
  // Safety net: if the viewport is resized up to desktop width while the
  // panel happens to be open, force it closed so it can never linger.
  window.addEventListener("resize", () => {
    if (window.innerWidth >= 900 && mobilePanel.classList.contains("is-open")) {
      closeMobilePanel();
    }
  });
  ...
  /* ---------- Contact form: validation + Formspree + GA4 ---------- */
  const form = document.getElementById("contact-form");
  if (form) {
    let startTracked = false;
    const formStatus = document.getElementById("form-status");
    const formspreeEndpoint = form.action; // https://formspree.io/f/mqeorzdr
    ...
    form.addEventListener("submit", async (e) => {
      ...
      if (!firstName || !email || !goal) {
        formStatus.textContent = "Please fill in the required fields (first name, email, and goal).";
      ...
      try {
        // Formspree expects multipart/FormData directly, not JSON.
        ...
        CoachFraAnalytics.trackFormSubmit({ goal });
        form.reset();
        startTracked = false;
      } catch (err) {
